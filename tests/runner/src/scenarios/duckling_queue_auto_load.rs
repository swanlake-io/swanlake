...
pub async fn run_duckling_queue_auto_load(args: &CliArgs) -> Result<()> {
    ...
    conn1.exec("DROP TABLE IF EXISTS swanlake.auto_load_test;").ok();
    ...
    assert_eq!(count, 1, "Should have 1 row in duckling_queue.auto_load_test");
    ...
    assert_eq!(total, 2, "Should have 2 rows in swanlake.auto_load_test after flush");
