...
pub async fn run_duckling_queue_auto_load(args: &CliArgs) -> Result<()> {
    ...
    let mut conn1 = FlightSQLClient::connect(args.endpoint())?;
    conn1.execute_update(&attach_sql)?;

    // Clean up any existing table
    conn1
        .execute_update("DROP TABLE IF EXISTS swanlake.auto_load_test;")
        .ok();

    // Create table in target schema
    conn1.execute_update("CREATE TABLE swanlake.auto_load_test (id INTEGER, value VARCHAR);")?;
    conn1.execute_update("INSERT INTO swanlake.auto_load_test VALUES (1, 'initial');")?;
    ...
    let mut conn2 = FlightSQLClient::connect(args.endpoint())?;
    conn2.execute_update(&attach_sql)?;

    let count = conn2.query_scalar_i64("SELECT COUNT(*) FROM swanlake.auto_load_test")?;
    ...
    conn2.execute_update("INSERT INTO duckling_queue.auto_load_test VALUES (2, 'from_queue');")?;

    // Verify the data is in duckling_queue
    let count = conn2.query_scalar_i64("SELECT COUNT(*) FROM duckling_queue.auto_load_test")?;
    assert_eq!(
        count, 1,
    ...
    // Flush to target
    conn2.execute_update("PRAGMA duckling_queue.flush;")?;

    // Verify both records are in the target schema
    let total = conn2.query_scalar_i64("SELECT COUNT(*) FROM swanlake.auto_load_test")?;
    assert_eq!(
        total, 2,
    ...
    // Clean up
    conn2.execute_update("DROP TABLE IF EXISTS swanlake.auto_load_test;")?;
