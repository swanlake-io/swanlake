...
pub fn run_prepared_statements(args: &CliArgs) -> Result<()> {
...
impl<'a> PreparedStatementTester<'a> {
    ...
    fn test_parameter_mismatch_errors(&mut self) -> Result<()>{
       ...
       let test_result = (|| -> Result<()>{
        ...
        let single_param_schema = Arc::new(Schema::new(vec![Field::new("col1", DataType::Int32, false)]));
        let single_param_batch = RecordBatch::try_new(single_param_schema, vec![Arc::new(Int32Array::from(vec![1])) as ArrayRef],)?;
        let result = self.client.update_with_record_batch("INSERT INTO param_mismatch_test (col1, col2) VALUES (?, ?)", single_param_batch);
        ...
        let result = self.client.update_with_record_batch("INSERT INTO param_mismatch_test (col1) VALUES (?)", double_param_batch);
        assert!(result.is_err());
        Ok(())
       })();
    ...
    fn test_null_empty_parameters(&mut self) -> Result<()>{
        ...
        let test_result = (|| -> Result<()>{
            ...
            let null_batch = RecordBatch::try_new(null_schema, vec![
                Arc::new(Int32Array::from(vec![1])) as ArrayRef,
                Arc::new(StringArray::from(vec![Option::<&str>::None])) as ArrayRef,
                Arc::new(Int32Array::from(vec![Option::<i32>::None])) as ArrayRef,
            ])?;

            self.client.update_with_record_batch("INSERT INTO null_empty_test (id, name, value) VALUES (?, ?, ?)", null_batch)?;

            //verify null values were inserted
            let result = self.client.query("SELECT name, value FROM null_empty_test WHERE id = 1")?;
            let batch = result.batches.first().context("expected row after null insert")?;
    ...
    fn test_ephemeral_prepared_statements(&mut self) -> Result<()>{
        ...
        let test_result = (|| -> Result<()>{
            ...
            let regular_result = self.client.query_with_param("SELECT name FROM ephemeral_test WHERE id = ?",
             regular_params)?;
