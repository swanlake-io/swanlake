...
BENCHBASE_CACHE_HTTPFS_DIRECTORY="${BENCHBASE_CACHE_HTTPFS_DIRECTORY:-$WORK_DIR/cache_httpfs-$(date +%Y%m%d-%H%M%S)}"
BENCHBASE_CACHE_HTTPFS_DISK_READER_MEMORY_CACHE="${BENCHBASE_CACHE_HTTPFS_DISK_READER_MEMORY_CACHE:-false}"
BENCHBASE_CACHE_HTTPFS_ENABLE_CACHE_VALIDATION="${BENCHBASE_CACHE_HTTPFS_ENABLE_CACHE_VALIDATION:-true}"
...
if [[ "$BENCHBASE_ENABLE_CACHE_HTTPFS" == "true" ]]; then
  if [[ "${SWANLAKE_DUCKLAKE_INIT_SQL:-}" == *"cache_httpfs"* ]]; then
    ...
    mkdir -p "$BENCHBASE_CACHE_HTTPFS_DIRECTORY"
    CACHE_HTTPFS_DIR_SQL="$(printf "%s" "$BENCHBASE_CACHE_HTTPFS_DIRECTORY" | sed "s/'/''/g")"
    export SWANLAKE_DUCKLAKE_INIT_SQL="INSTALL cache_httpfs FROM community; LOAD cache_httpfs; SET cache_httpfs_cache_directory='${CACHE_HTTPFS_DIR_SQL}'; SET cache_httpfs_disk_cache_reader_enable_memory_cache=${BENCHBASE_CACHE_HTTPFS_DISK_READER_MEMORY_CACHE}; SET cache_httpfs_enable_cache_validation=${BENCHBASE_CACHE_HTTPFS_ENABLE_CACHE_VALIDATION}; ${SWANLAKE_DUCKLAKE_INIT_SQL:-}"
    log "Enabled cache_httpfs (dir=$BENCHBASE_CACHE_HTTPFS_DIRECTORY, disk_reader_memory_cache=$BENCHBASE_CACHE_HTTPFS_DISK_READER_MEMORY_CACHE, cache_validation=$BENCHBASE_CACHE_HTTPFS_ENABLE_CACHE_VALIDATION)"
  fi
fi
