...
impl SwanFlightSqlService {
    ...
    pub(crate) async fn get_session<T>(&self, request: &Request<T>) -> Result<Arc<Session>, Status> {
        let session_id = Self::extract_session_id(request);
        self.registry
            .get_or_create_by_id(&session_id).await
    ...
    pub(crate) async fn prepare_request<T>(&self, request: &Request<T>) -> Result<Arc<Session>, Status> {
        let session_id = Self::extract_session_id(request);
